...
def setPlot(obsDb,prjName,pltFname,relParam=None,cFactor=1.,tFactor=1.,cCut = 1.e-4,fltDb=None):
  ...
  obsConc = db2Array(obsCur,'SELECT xStn,yStn,EpTime,conc from obsTable where \
                             conc >= 0. order by xStn,yStn,EpTime')
  cOmax = max(obsConc[:,3])
  print 'Max Obs Conc = ',cOmax
  
  # Predicted concentrations 
  preConc = db2Array(preCur,'SELECT a.xSmp,a.ySmp,p.EpTime,p.value from smpTable p,samtable a where \
